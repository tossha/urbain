const SOLAR_SYSTEM_BARYCENTER = 0;
const SUN                     = 10;
const MERCURY_BARYCENTER = 1;
const VENUS_BARYCENTER   = 2;
const EARTH_BARYCENTER   = 3;
const MARS_BARYCENTER    = 4;
const JUPITER_BARYCENTER = 5;
const SATURN_BARYCENTER  = 6;
const URANUS_BARYCENTER  = 7;
const NEPTUNE_BARYCENTER = 8;
const PLUTO_BARYCENTER   = 9;
const MERCURY = 199;
const VENUS   = 299;
const EARTH   = 399;
const MARS    = 499;
const JUPITER = 599;
const SATURN  = 699;
const URANUS  = 799;
const NEPTUNE = 899;
const PLUTO   = 999;

const MOON = 301;

const RF_TYPE_ECLIPTIC = 1; // reference frame type: inertial
const RF_TYPE_EQUATORIAL = 2;
const RF_TYPE_ROTATING = 3; // reference frame type: rotating with the body
const RF_TYPE_ECI = 4; // Earth centered inertial
const RF_TYPE_ICRF = 5; // International celestial reference frame

const VM_SHAPE_SPHERE = 1; // visual model shape type: sphere
const VM_SHAPE_MODEL  = 2; // visual model shape type: custom 3d model

const COMMON_TEXTURE_PATH = 'texture/';
