...
export default class TrajectoryELP2000 extends TrajectoryKeplerianAbstract {
    ...
    getStateInOwnFrameByEpoch(epoch) {
        const model = this._calcAll(epoch / SECONDS_PER_DAY / 36525);
        ...
        return new StateVector(
            new Vector([model.r * cosLon * cosLat, model.r * sinLon * cosLat, model.r * sinLat]),
            new Vector([
                (model.dr * cosLon * cosLat -
                    model.r * sinLon * cosLat * model.dlon -
                    model.r * cosLon * sinLat * model.dlat) /
                    36525 /
                    SECONDS_PER_DAY,
                (model.dr * sinLon * cosLat +
                    model.r * cosLon * cosLat * model.dlon -
                    model.r * sinLon * sinLat * model.dlat) /
                    36525 /
                    SECONDS_PER_DAY,
                (model.dr * sinLat + model.r * cosLat * model.dlat) / 36525 / SECONDS_PER_DAY,
            ]),
        );
    }
    ...
    _calcAll(t) {
        ...
        this.W1 =
            218 * 3600 + 18 * 60 + 59.95571 + 1732559343.73604 * t - 5.8883 * t2 + 0.006604 * t3 - 0.00003169 * t4;
